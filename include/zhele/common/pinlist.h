...
class PinList : public detail::IoSpeed<template_utils::type_unbox<template_utils::type_list<typename _Pins::Port...>::remove_duplicates().head()>>,
                public detail::IoDriverType<template_utils::type_unbox<template_utils::type_list<typename _Pins::Port...>::remove_duplicates().head()>> {
...
public:
    ...
    // DriverType — from IoDriver<FirstPort> when defined
    static constexpr bool supports_speed  = requires { typename FirstPort::Speed; };
    static constexpr bool supports_driver = requires { typename FirstPort::DriverType; };
    ...
    static void SetDriverType(auto driver, DataType mask = std::numeric_limits<DataType>::max())
        requires supports_driver;

    template <auto driver, DataType mask = std::numeric_limits<DataType>::max()>
