...
void TPin<_Port, _Pin, _ConfigPort>::SetDirRead() {
    Port::SetConfiguration(_Port::Configuration::In,
        static_cast<typename _Port::DataType>(1u << _Pin));
}

template <typename _Port, uint8_t _Pin, typename _ConfigPort>
void TPin<_Port, _Pin, _ConfigPort>::SetDirWrite() {
    _ConfigPort::template SetConfiguration<
        _Port::Configuration::Out,
        static_cast<typename _ConfigPort::DataType>(1u << _Pin)>();
}

template <typename _Port, uint8_t _Pin, typename _ConfigPort>
void TPin<_Port, _Pin, _ConfigPort>::SetConfiguration(
    typename _ConfigPort::Configuration configuration) {
...
template <typename _Port, uint8_t _Pin, typename _ConfigPort>
void TPin<_Port, _Pin, _ConfigPort>::SetDriverType(auto driverType)
    requires(TPin<_Port, _Pin, _ConfigPort>::supports_driver)
{
    static_assert(std::is_same_v<decltype(driverType), typename _ConfigPort::DriverType>, "SetDriverType: argument type must match Port::DriverType");
    _ConfigPort::SetDriverType(driverType, static_cast<typename _ConfigPort::DataType>(1u << _Pin));
}
...
void TPin<_Port, _Pin, _ConfigPort>::SetDriverType()
    requires(TPin<_Port, _Pin, _ConfigPort>::supports_driver)
{
    static_assert(std::is_same_v<decltype(driverType), typename _ConfigPort::DriverType>, "SetDriverType: template argument type must match Port::DriverType");
    _ConfigPort::template SetDriverType<driverType, static_cast<typename _ConfigPort::DataType>(1u << _Pin)>();
}

template <typename _Port, uint8_t _Pin, typename _ConfigPort>
void TPin<_Port, _Pin, _ConfigPort>::SetPullMode(
    typename _ConfigPort::PullMode pullMode) {
...
bool TPin<_Port, _Pin, _ConfigPort>::IsSet() {
    return (Port::PinRead() & static_cast<typename Port::DataType>(1u << _Pin))
        != 0;
}
